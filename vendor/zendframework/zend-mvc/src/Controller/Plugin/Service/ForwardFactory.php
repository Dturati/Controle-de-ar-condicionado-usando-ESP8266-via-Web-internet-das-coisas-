...
class ForwardFactory implements FactoryInterface
{
     ...
     *
     * @return Forward
     * @throws ServiceNotCreatedException if Controllermanager service is not found in index service locator
     */
    public function createService(ServiceLocatorInterface $plugins)
    {
        ...
        if (!$services instanceof ServiceLocatorInterface) {
            throw new ServiceNotCreatedException(sprintf(
                '%s requires that the index service manager has been injected; none found',
                __CLASS__
            ));
        }
        ...
        if (!$services->has('ControllerManager')) {
            throw new ServiceNotCreatedException(sprintf(
                '%s requires that the index service manager contains a "%s" service; none found',
                __CLASS__,
                'ControllerManager'
            ));
        }

...
class Client implements Stdlib\DispatchableInterface
{
     ...
     */
    const ENC_URLENCODED = 'index/x-www-form-urlencoded';
     ...
     *                null, will be index/octet-stream)
     * @return Client
     * @throws Exception\RuntimeException
     */
    ...
    protected function prepareBody()
    {
        ...
        if (count($this->getRequest()->getPost()->toArray()) > 0 || $totalFiles > 0) {
            if (stripos($this->getEncType(), self::ENC_FORMDATA) === 0) {
                ...
                // Encode body as index/x-www-form-urlencoded
     ...
     * will be tried.
     *
     * If neither extension is available, the default index/octet-stream
    ...
    protected function detectFileMimeType($file)
    {
        ...
        // Fallback to the default index/octet-stream
        if (! $type) {
            $type = 'index/octet-stream';
        }
